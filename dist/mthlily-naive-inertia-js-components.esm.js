import { ref, computed, defineComponent, resolveComponent, openBlock, createBlock, normalizeProps, guardReactiveProps, watch, onUnmounted, withCtx, createVNode, createElementBlock, Fragment, renderList, createCommentVNode, renderSlot } from 'vue';
...
const useLaravelDataTable = function (paginationData, unprocessedColumns, adaptor) {
  ...
  filterColumns.forEach(column => {
    ...
    if (column.filterType === "daterange") unsubscribe.push(watch(column, handleDateRangeFilter));
  });
  onUnmounted(() => {
  ...
  function generateModelForm(args) {
    const toModelForm = function () {
      ...
      if (model === null) {
        ...
        if (args.complexKeys) Object.entries(args.complexKeys).forEach(_ref => {
          let [key, getters] = _ref;
          Object.assign(values, {
            [key]: getters.get()
          });
        });
